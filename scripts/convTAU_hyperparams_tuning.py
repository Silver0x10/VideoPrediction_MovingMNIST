...
training_dataloader = DataLoader(
    train_set, batch_size=params_ConvTAU['batch_size'])
validation_dataloader = DataLoader(
    validation_set, batch_size=params_ConvTAU['batch_size'])
...
for key in tuned_values.keys():
    for val in tuned_values[key]:
        ...
        # logger=wandb_logger
        trainer = pl.Trainer(max_epochs=3, accelerator=device.type)
        trainer.fit(model=model_convTAU, train_dataloaders=training_dataloader,
...
for test in results:
    out = str(test['param']) + "\t" + str(test['value']) + \
        "\t" + str(test['loss']) + "\n"
    print(out)
